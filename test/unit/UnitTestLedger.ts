...
const gasPrice = new BN(100000000);
const requiredDeposit = new BN(10000000);
const opts = {
  to: account1,
  value: new BN(0),
  gas,
...
describe('Ledger Unit Tests', async () => {
  ...
  it('should account for bounty only when execution was successful', async () => {
    ...
    const gasUsed = new BN(receipt.object.gasUsed);
    const actualGasPrice = opts.gasPrice;
    const expectedCost = new BN(0);
    const expectedReward = new BN(
      '000000000000000000000000000000000000000000000000000fe3c87f4b7363',
      16
